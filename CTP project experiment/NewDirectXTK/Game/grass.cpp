...
void VBGrass::init(int _size, ID3D11Device* GD)
{
	m_size = _size;
	//////modded for single blade/////
	...
	//{
		...
		//m_vertices[2].Pos = A1; //Vector3((float)(i+ 1), (float)(j + 1), 0.5f * (float)(m_size - 1));

		//m_vertices[3].Color = Color(0.0f, 1.0f, 0.0f, 1.0f);
		//m_vertices[3].Pos = Vector3((float)(i + 0), 30, 0.5f * (float)(m_size - 1));// Vector3((float)(i + 1), (float)j, 0.5f * (float)(m_size - 1));
...
void VBGrass::Tick(GameData* _GD)
{
...
//	Transform();
		A1 += Vector3(10, 0, 0);
		//m_vertices[2].Pos += Vector3(10, 0, 0)* _GD->m_dt;//Vector3(_GD->m_dt, 0, 0);
