...
Game::Game(ID3D11Device* _pd3dDevice, HWND _hWnd, HINSTANCE _hInstance) :m_playTime(0), m_fxFactory(nullptr), m_states(nullptr)
{
	...
	pPlayer->SetScale(1.01f);
	pPlayer->SetPos(Vector3(-10.0f, -10.0f, 20.0f));
  m_GameObjects.push_back(pPlayer);
	...
	//HUD stuff
	GameData GameData;
	float HUDK = GameData.K;
	float NLEN = GameData.NaturalLength;
	TwInit(TW_DIRECT3D11, _pd3dDevice); // for Direct3D 11
	TwWindowSize(width, height);

	TwBar *mBar;
	mBar = TwNewBar("HUD");
	TwAddVarRW(mBar, "Tension", TW_TYPE_FLOAT ,&HUDK,"min = 0.1 max=0.9 lable='spring tension'");
	TwAddVarRW(mBar, "Natural length", TW_TYPE_FLOAT, &NLEN, "min = 4.0 max=11.0 lable='spring rest length'");
